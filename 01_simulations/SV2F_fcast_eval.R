...
filename <- paste0("fcasts_", Symbol, "_M", M, ".rds")
...
avgdiff <-
  fcasts %>% 
  filter(model == "HAR") %>% 
  group_by(sym, win, horizon) %>% 
  summarise(mean(coef.diff, na.rm=T)) %>% arrange(sym)
avgdiff

filename <- paste0("coeffdiff_HAR_", Symbol, "_M", M, ".rds")

#saveRDS(avgdiff, filename)
...
filename <- paste0("coeffdiff_SHAR_", Symbol, "_M", M, ".rds")
...
filename <- paste0("DMW_results_individual_", Symbol, "_M", M, ".rds")
